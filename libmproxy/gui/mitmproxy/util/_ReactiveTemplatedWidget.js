...
define([
    ...
    var bindInput = function (type, node, value) {
        var isSimple = (typeof(value[1]) === "string");
        var getter, setter;
        if (isSimple) {
            getter = function () {
            ...
            setter = function (evt) {
                var oldValue = value[0][value[1]];
                value[0][value[1]] = node.value;
                if (value[0].notify)
                    value[0].notify({
                        type: "updated",
                        name: value[1],
        ...
        var val = getter();
        node.value = val;
        eventListenerBinding.call(this, "change", node, setter);
    };
    default_bindings.bindInput = bindInput;
